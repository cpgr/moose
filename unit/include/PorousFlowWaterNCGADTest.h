...
#ifndef POROUSFLOWWATERNCGADTEST_H
#define POROUSFLOWWATERNCGADTEST_H
...
class PorousFlowWaterNCGADTest : public MooseObjectUnitTest
{
public:
  PorousFlowWaterNCGADTest() : MooseObjectUnitTest("MooseUnitApp") { buildObjects(); }
...
protected:
  void buildObjects()
  {
    ...
    InputParameters uo_params = _factory.getValidParams("PorousFlowWaterNCGAD");
    ...
    _fe_problem->addUserObject("PorousFlowWaterNCGAD", "fp", uo_params);
    _fp = &_fe_problem->getUserObject<PorousFlowWaterNCGAD>("fp");
  }

  const PorousFlowCapillaryPressureVG * _pc;
  const PorousFlowWaterNCGAD * _fp;
  const SinglePhaseFluidProperties * _water_fp;
  const SinglePhaseFluidProperties * _ncg_fp;
};

#endif // POROUSFLOWWATERNCGADTEST_H
