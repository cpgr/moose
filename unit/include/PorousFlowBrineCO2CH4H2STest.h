...
#ifndef POROUSFLOWBRINECO2CH4H2STEST_H
#define POROUSFLOWBRINECO2CH4H2STEST_H
...
class PorousFlowBrineCO2CH4H2STest : public MooseObjectUnitTest
{
public:
  PorousFlowBrineCO2CH4H2STest() : MooseObjectUnitTest("MooseUnitApp") { buildObjects(); }
...
protected:
  void buildObjects()
  {
    ...
    InputParameters ch4_params = _factory.getValidParams("MethaneFluidProperties");
    _fe_problem->addUserObject("MethaneFluidProperties", "ch4_fp", ch4_params);
    _ch4_fp = &_fe_problem->getUserObject<MethaneFluidProperties>("ch4_fp");

    InputParameters uo_params = _factory.getValidParams("PorousFlowBrineCO2CH4H2S");
    uo_params.set<UserObjectName>("brine_fp") = "brine_fp";
    uo_params.set<UserObjectName>("co2_fp") = "co2_fp";
    uo_params.set<UserObjectName>("ch4_fp") = "ch4_fp";
    uo_params.set<UserObjectName>("capillary_pressure") = "pc";
    _fe_problem->addUserObject("PorousFlowBrineCO2CH4H2S", "fp", uo_params);
    _fp = &_fe_problem->getUserObject<PorousFlowBrineCO2CH4H2S>("fp");
  }

  const PorousFlowCapillaryPressureVG * _pc;
  const PorousFlowBrineCO2CH4H2S * _fp;
  const BrineFluidProperties * _brine_fp;
  const Water97FluidProperties * _water_fp;
  const CO2FluidProperties * _co2_fp;
  const MethaneFluidProperties * _ch4_fp;
};

#endif // POROUSFLOWBRINECO2CH4H2STEST_H
