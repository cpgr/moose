...
PorousFlowPorosityExponentialBase::validParams()
{
  ...
  params.addParam<Real>(
      ...
      "porosity is set to this value instead.  By default no floor is imposed.  The ensure_positive "
      "transform only acts for decay > 0, so chemistry-driven (precipitation) porosity is otherwise "
  ...
  params.addParam<Real>(
      "zero_modifier",
      1E-3,
      "If the porosity_min floor is active, the porosity derivatives are set to zero_modifier times "
      "their unfloored values (rather than exactly zero) to hint to the Newton process that porosity "
